...
### 言語別コーディング規約
-   **Python**: 型ヒントを必須とし、最大行長は88文字以内
-   **C#**: 明示的型宣言を推奨し、XMLドキュメントコメントを記述
-   **TypeScript**: 厳密な型定義を行い、ESLint・Prettierの設定に従う
-   **JavaScript**: 可能な限りTypeScriptの使用を推奨
...
-   **.NET バージョン**: 最新の LTS バージョン（.NET 8以降）を使用すること
-   **nullable参照型**: 必ず有効にし、適切にnull安全性を確保すること
-   **依存性注入**: Microsoft.Extensions.DependencyInjectionを積極的に活用すること
-   **非同期処理**: I/O集約的な処理では必ずasync/awaitパターンを使用すること
-   **パフォーマンス**: メモリ効率とガベージコレクションを考慮した実装を行うこと

### React×TypeScript 固有の要件

-   **プロジェクト構築**: React×TypeScript プロジェクトは必ず Vite で構築すること。
-   **Tailwind CSS v4 対応**: Tailwind CSS v4 を使用する場合の設定
    -   Vite のバージョンは v6.～ を使用すること。
    -   `@apply`ディレクティブは非推奨のため使用を避けること。
    -   Tailwind CSS v4 の新しい構文とベストプラクティスに従うこと。
...
-   Conventional Commits形式に従うこと（feat:, fix:, docs:, refactor:, test: など）
-   コミットメッセージには Claude 等の生成ツール名を**一切記載しないこと**
-   例外として、ユーザーが明示的に指示した場合のみ生成ツール名の記載を許可する

### ブランチ名のルール
-   **Issue対応の場合**: `feature/issue-[issue番号]` の形式を使用すること
...
### プルリクエスト作成時のルール
-   **Issue対応の場合**: プルリクエスト本文に `fixes #[issue番号]` を必ず含めること
    -   マージ時にIssueが自動的にクローズされる
    -   例: `fixes #1`, `fixes #123`
