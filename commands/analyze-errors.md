...
指定したプロジェクトのビルドエラーを分析し、修正計画を作成して GitHub Issue に登録します。

## コマンド形式

```
/create-bug-issue [プロジェクト名]
```
...
/create-bug-issue growth-diary

# value-meプロジェクトを解析
/create-bug-issue value-me

# portfolio-siteプロジェクトを解析
/create-bug-issue portfolio-site
```

## 実行手順

### フェーズ 1: エラー分析と md ファイル出力
...
4. 各エラーカテゴリごとに修正計画を md ファイルとして出力
...
生成された md ファイルをユーザーに確認してもらい、Issue 登録の承認を待つ。
...
### フェーズ 3: GitHub Issue 登録

ユーザーの承認後:

1. 各修正計画ファイルの内容を読み込み
2. GitHub Issue として登録（`mcp__github__create_issue`を使用）
    - タイトル: md ファイルの見出しから抽出
    - 本文: md ファイルの内容をそのまま使用
    - ラベル: 優先度とカテゴリに応じて設定
...
3. 登録完了後、Issue URL を表示
4. すべての Issue 登録が完了したら、生成した md ファイルをすべて削除
...
**注意**: プロジェクトの既存ファイル（README.md 等）は削除しないこと

**重要**: md ファイルの削除は、Issue 登録が完了した後のみ実行されます
...
| プロジェクトタイプ | 判定条件                                                              |
| ------------------ | --------------------------------------------------------------------- |
| Flutter            | `pubspec.yaml` + `lib/`ディレクトリ + Flutter 依存関係                |
| Dart               | `pubspec.yaml` + `lib/`ディレクトリ（Flutter 以外）                   |
...
### 例 1: カレントディレクトリを解析（引数なし）

```
ユーザー: /create-bug-issue
...
```
ユーザー: /create-bug-issue value-me
...
```
ユーザー: /create-bug-issue portfolio
...
```
ユーザー: /create-bug-issue nonexistent-project
...
-   様々なプロジェクトタイプに対応（Flutter, TypeScript, Python, Rust, Go 等）
-   プロジェクトタイプは自動判定されます
-   GitHub 認証情報が設定されている必要があります
-   大量のエラーがある場合、処理に時間がかかる可能性があります
-   md ファイルの内容を確認してから、Issue 登録を承認してください
