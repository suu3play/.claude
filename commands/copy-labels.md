...
## 概要
claude-configプロジェクトの標準ラベル設定を指定されたプロジェクトまたは全プロジェクトにコピーするスラッシュコマンド

## 実行手順

1. **対象プロジェクトの確認**
   - 個別指定: 指定プロジェクトがGitリポジトリであることを確認
   - all指定: 作業ディレクトリ配下を自動スキャンしてプロジェクト一覧を取得
   - プロジェクトタイプの自動判定（package.json, *.csproj等から判定）
   - gh CLIでアクセス可能であることを確認

2. **標準ラベル作成**
   - Type系：ドキュメント更新、バグ、リファクタリング、新機能・既存改修、good first issue、作業
   - Priority系：最優先、中、低
   - 色設定とラベル説明も統一適用
